...
T Mergesort(const T& data, int left, int right, const std::string& name, const std::string& opponent = "") {
    ...
    T Lsort = Mergesort<T, instance>(data, left, mid,  name, opponent);
    T Rsort = Mergesort<T, instance>(data,mid+1, right, name, opponent);
    Merge<T, instance>(subset, Lsort, Rsort, name, opponent);
    return subset;
}